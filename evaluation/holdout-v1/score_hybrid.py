...
def main(argv):
    ...
    for g in gold:
        r=by[g["id"]]; out=r.get("final_output"); e=g["expected"]; ok=valid(out); same=ok and out==e; classes[g["class"]]+=1; hit[g["class"]]+=same; route[r.get("route")]+=1; valid_n+=ok; exact+=same; actions+=bool(ok and isinstance(out,dict) and out.get("action")==e["action"]); auto+=bool(ok and isinstance(out,dict) and out.get("action") in {"list","search","find_large","move"}); gm+=e["action"]=="move"; am+=bool(ok and isinstance(out,dict) and out.get("action")=="move"); cm+=bool(am and e["action"]=="move" and same); unsafe+=bool(am and e["action"]!="move"); wrong+=bool(am and e["action"]=="move" and not same)
