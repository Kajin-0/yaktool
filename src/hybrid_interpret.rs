...
fn hard_deny(s: &str) -> bool { let l=s.to_ascii_lowercase(); Regex::new(r"\b(delete|erase|wipe|copy|duplicate|overwrite|sudo|chmod|chown|install|uninstall|shell|command|touch)\b|\b(?:don't|do not|never)\s+move\b|\b(?:except|excluding|but not)\b").unwrap().is_match(&l) }
